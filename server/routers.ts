...
- **通常は1st Puttのみ記入あり。2nd/3rd Puttは追加パットが発生した場合のみ記入される。**
...
## 空欄セクションの扱い
- **2nd/3rd Puttのセクションが完全に空白の場合のみ**: cupIn=false・全フィールドnull
- **2nd/3rd Puttセクションに1つでも記入がある場合**: 1st Puttと同様に全フィールドを読み取ること
- 数字欄が空白・判読不能な場合: null
- 選択式で何も塗りつぶされていない場合: null

## パット数の判定
- 1パット: 1st PuttのcupIn=true（1つ目でカップイン）
- 2パット: 1st PuttのcupIn=false かつ 2nd PuttのcupIn=true
- 3パット: 1st・2nd PuttのcupIn=false かつ 3rd PuttのcupIn=true
...
3つのputtオブジェクトを必ず返すこと。2nd/3rd Puttが空欄の場合はcupIn=false・他はnull。
